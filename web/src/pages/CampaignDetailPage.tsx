...
export function CampaignDetailPage() {
  ...
  const reviewedMaterials = allMaterials.filter((m) => m.review);
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
        ...
        <aside className="lg:order-2 lg:sticky lg:top-6 lg:w-80 lg:shrink-0 space-y-3">
          ...
          {reviewedMaterials.length > 0 ? (
            <div className="surface rounded-2xl p-4">
              <p className="eyebrow">Reports &amp; chats</p>
              <p className="mt-1 text-[11px] text-faint">Saved per reviewed material. Survives restarts.</p>
              <ul className="mt-2 space-y-2">
                {reviewedMaterials.map((m) => {
                  const reportId = m.review?.reportArtifactId;
                  const transcriptId = m.review?.transcriptArtifactId;
                  return (
                    <li key={m.id} className="rounded-lg border border-border bg-bg-soft/40 px-2.5 py-2">
                      ...
                      <div className="mt-1.5 flex gap-1.5">
                        {reportId ? (
                          <button
                            type="button"
                            onClick={() => setReportArtifactId(reportId)}
                            className={`rounded-md border border-border px-2 py-1 text-[11px] transition-colors hover:bg-accent/[0.08] ${reportArtifactId === reportId ? 'bg-accent/10 text-fg' : 'text-muted'}`}
                          >
                            Report
                          </button>
                        ) : null}
                        {transcriptId ? (
                          <button
                            type="button"
                            onClick={() => setReportArtifactId(transcriptId)}
                            className={`rounded-md border border-border px-2 py-1 text-[11px] transition-colors hover:bg-accent/[0.08] ${reportArtifactId === transcriptId ? 'bg-accent/10 text-fg' : 'text-muted'}`}
                          >
                            Chat
                          </button>
                        ) : (
      ...
      {reportArtifactId ? (
        <div className="fixed left-4 top-20 bottom-6 z-40 w-[calc(100%-2rem)] max-w-2xl sm:left-8 sm:w-[40rem]">
          <div className="surface flex h-full flex-col overflow-hidden rounded-2xl border border-border shadow-2xl">
            <div className="glass flex items-center justify-end border-b border-border px-5 py-3">
