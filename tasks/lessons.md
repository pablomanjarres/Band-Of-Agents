...
## 2026-06-19: Do NOT verify CDN-proxied media with tiny Range probes (poisons Vercel cache)
- Observation: verified hosted videos with `curl -r 0-1 https://<vercel-site>/api/videos/<id>.mp4`. The backend serves video with `Cache-Control: public, max-age=31536000, immutable` on BOTH 200 and 206. Vercel's edge cached the 2-byte 206 body keyed by URL (cache key ignored Range) and then served those 2 bytes as `x-vercel-cache: HIT` for ALL later requests, including the browser's. Result: `<video>` got a 2-byte file -> "moov atom not found" -> onError -> "Video preview unavailable" fallback. The file, codec (h264/High/yuv420p+aac), and backend were all fine; only the poisoned CDN entry was wrong. Real browsers request `Range: bytes=0-` (the whole file), so they never trigger this; my tiny-range probe did.
- Rule: when checking proxied/CDN media, fetch a FULL GET (no Range) or `Range: bytes=0-` (open-ended = whole file). Never probe with a small byte range like `0-1` / `0-1023` against a URL whose responses are `immutable`. To bust an already-poisoned entry, re-upload to a fresh UUID URL (the old one is abandoned) and prime it with a full GET. Root-cause hardening (needs a Cloud Run redeploy): do not send a long-lived `immutable` cache header on 206 partial responses in src/server/index.ts; only cache the full 200.
