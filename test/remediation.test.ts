...
describe('Remediation loop: an adapt verdict triggers a rewrite + regenerated image', () => {
  it('reconcile routes the adapt region to remediation, which posts a revised asset', async () => {
    ...
    await room.connectAgent({
      ...
      name: 'Remediation',
      handle: '@remediation',
      onMessage: makeRemediation({ brand, copyModel, imageModel, reportToHandle: '@coordinator' }),
    });
    await room.connectAgent({
      agentId: 'rec',
      name: 'Reconcile',
      handle: '@reconcile',
      onMessage: makeReconcile({ expectedRegions: ['EU'], coordinatorHandle: '@coordinator', remediationHandle: '@remediation' }),
    });

    room.post('lead', JSON.stringify(asset), [{ id: 'coord' }]);
    await room.drain();

    const revisedMsg = room.transcript.find((t) => t.fromId === 'rem' && t.kind === 'message');
    expect(revisedMsg).toBeDefined();
    const payload = JSON.parse(revisedMsg!.content) as {
      kind: string;
      region: string;
      revised: { copy: string; imageUrl?: string; markets: string[] };
    };
    expect(payload.kind).toBe('revised');
    expect(payload.region).toBe('EU');
    expect(payload.revised.copy).toContain('balanced diet');
    expect(payload.revised.imageUrl).toBe('https://cdn.aimlapi.com/revised-eu.png');
    expect(payload.revised.markets).toEqual(['EU']);
  });
});
