...
function assetMsg(content: string): RoomMessage {
  return { id: 'm', roomId: 'r', content, senderId: 'coord', senderType: 'agent', senderName: 'Coordinator', messageType: 'chat', mentions: [], metadata: {}, createdAt: new Date() };
}

const ASSET = JSON.stringify({ id: 'a1', channel: 'post', markets: ['US'], copy: 'c', claim: 'c' });
...
describe('precedent -> shared context (closes the logs-precedent loop)', () => {
  it('feeds recent human-decision precedents into the region reviewer prompt', async () => {
    ...
    await handler(assetMsg(ASSET), tools(), { roomId: 'r', agentId: 'us', agentName: 'US Reviewer' });
  ...
  it('omits the precedent block when there are none', async () => {
    ...
    const handler = makeRegionReviewer({ region: 'US', reviewerName: 'US Reviewer', rulebook: usRules, brand, model, reportToHandle: '@reconcile', precedents: () => [] });
    await handler(assetMsg(ASSET), tools(), { roomId: 'r', agentId: 'us', agentName: 'US Reviewer' });
